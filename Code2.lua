...
local config = {
    ...
    TOOL_TIERS = {"wood_pickaxe", "stone_pickaxe", "iron_pickaxe", "diamond_pickaxe", "wood_axe", "stone_axe", "iron_axe", "diamond_axe"},
    TOXIC_RESPONSES = {
...
local function switchToTool(toolName)
    ...
    for _, target in ipairs(Workspace:GetPartsInPart(root, config.NUKE_RADIUS)) do
        ...
        if parent then
            if parent.Name == "bed" then
                for _, toolTier in ipairs(config.TOOL_TIERS) do
                    switchToTool(toolTier)
